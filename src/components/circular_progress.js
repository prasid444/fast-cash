...
const CircularProgress = ({percent, radius, bgRingWidth, progressRingWidth, ringColor, ringBgColor,
  textFontSize, textFontWeight, clockwise, bgColor, startDegrees,renderChild,child}) => {
  const commonStyles = {
    width: radius * 2,
    height: radius * 2,
    borderRadius: radius
  };
  const childCommonStyles = {
    width: radius * 2-bgRingWidth,
    height: radius * 2 -bgRingWidth,
    borderRadius: radius
  };
  ...
  */
  ...
  return(
    <View style={[styles.container, {width: radius * 2, height: radius * 2}]}>
      ...
      <View style={{...styles.display,...childCommonStyles,overflow:'hidden'}}>
          {renderChild()}
      </View>
      :
